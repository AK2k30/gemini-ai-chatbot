...
export function ChatMessage({ message, ...props }: ChatMessageProps) {
  // Function to extract the email content from the message
  const extractEmailContent = (content: string) => {
  ...
  const emailContent = extractEmailContent(message.content);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 px-1 ml-4 space-y-2 overflow-hidden">
        ...
        {message.role === 'assistant' && emailContent && (
          <SendEmailButton generatedEmail={emailContent} />
        )}
      </div>
    </div>
  );
